...
represent the left-hand interval limits. The resultant groupings will span from
the minimum break through to `Inf` and will always be closed on the left and
open on the right. Ages below the minimum break will be returned as NA.
As an example, if `breaks = c(0, 1, 10, 30)` the possible groupings would be
"[0, 1)", "[1, 10)", "[10, 30)" and "[30, Inf)". This is roughly comparable to a
call of `cut(ages, right = FALSE, breaks = c(limits, Inf))` but with both the
resultant interval and the start and end points returned as entries in a list.
